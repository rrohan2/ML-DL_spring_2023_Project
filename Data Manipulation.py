...
###########################################################
import os
# copy paste the validation folder and rename it to test. this script will delete all the images in the test folder
test_manipulated_dir = 'C:\\Users\\bmaha\\PycharmProjects\\test123\\Data\\imagenet-mini\\test_manipulated'  # replace with the path to your test folder

# get a list of subdirectories in the test folder
subdirs = [name for name in os.listdir(test_manipulated_dir) if os.path.isdir(os.path.join(test_manipulated_dir, name))]
...
def save_img(image, imgname, use_JPEG=False):
    ...
    image_dest = os.path.join(subdir_path, imgname)
    # if use_JPEG:
    imsave(image_dest + ".JPEG", image)
    # else:
    #     toimage(image,
...
if __name__ == "__main__":
    ...
    for subdir in subdirs:
        subdir_path = os.path.join(test_manipulated_dir, subdir)

        # get a list of all image files in the subdirectory
        images = [name for name in os.listdir(subdir_path) if
                  os.path.isfile(os.path.join(subdir_path, name)) and name.endswith('.JPEG')]

        for image in images:
            image_path = os.path.join(subdir_path, image)
            use_JPEG = False  # either JPEG or PNG
            img = imload_rgb(image_path)
            ...
            save_img(img_grayscale, image+"_grayscale", use_JPEG)
            ...
            save_img(img_low_contrast, image+"_low_contrast", use_JPEG)
            ...
            img_noisy = uniform_noise(image=img, width=noise_width,
                                      ...
                                      rng=rng)
            save_img(img_noisy, image+"_noisy", use_JPEG)
            ...
            img_noisy = salt_and_pepper_noise(image=img,
                                              ...
                                              rng=rng)
            save_img(img_noisy, image+"_salt_and_pepper_noise", use_JPEG)
            ...
            save_img(img_false_colour, image+"_false-colour", use_JPEG)
            ...
            save_img(img_highpass, image+"_highpass", use_JPEG)
            ...
            save_img(img_lowpass, image+"_lowpass", use_JPEG)
            ...
            save_img(img_rotation90, image+"_rotation90", use_JPEG)

            img_rotation180 = rotate180(img)
            save_img(img_rotation180, image+"_rotation180", use_JPEG)

            img_rotation270 = rotate270(img)
            save_img(img_rotation270, image+"_rotation270", use_JPEG)
